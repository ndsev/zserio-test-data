package structure_types;

import structure_types.empty_structure.*;
import structure_types.empty_structure_with_parameter.*;
import structure_types.field_constructor_clashing.*;
import structure_types.one_string_structure.*;
import structure_types.simple_structure.*;
import structure_types.structure_inner_classes_clashing.*;
import structure_types.structure_with_removed_enum_item_field.*;
import structure_types.templated_structure.*;
